...
func TestOrganizationUserService_GetUsersByOrganizationID(t *testing.T) {
	...
	t.Run("successful retrieval", func(t *testing.T) {
		users, err := service.GetUsersByOrganizationID(context.Background(), uuid.New().String(), uuid.New().String())
		assert.NoError(t, err)
	...
	t.Run("invalid organization ID", func(t *testing.T) {
		_, err := service.GetUsersByOrganizationID(context.Background(), "invalid-id", uuid.New().String())
		assert.Error(t, err)
	})
	t.Run("empty organization ID", func(t *testing.T) {
		_, err := service.GetUsersByOrganizationID(context.Background(), "", uuid.New().String())
		assert.Error(t, err)
	})
	t.Run("user not part of organization", func(t *testing.T) {
		mockRepo.GetByIDFunc = func(ctx context.Context, orgID string, userID string) (*models.OrganizationUser, error) {
		...
		_, err := service.GetUsersByOrganizationID(context.Background(), uuid.New().String(), uuid.New().String())
		assert.Error(t, err)
		assert.Equal(t, services.ErrUserNotPartOfOrganization, err)
	})
}
